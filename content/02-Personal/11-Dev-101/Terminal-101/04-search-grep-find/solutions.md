...
[[02-Personal/11-Dev-101/Terminal-101/04-search-grep-find/index\|back to the chapter]]
...
- notice in step 1 the first line uses `>` and the rest use `>>`. the first one creates the file, the rest append to it. if u used `>` every time u would end up with only the last line. this is [[02-Personal/11-Dev-101/Terminal-101/08-io-pipes-processes/index\|Chapter 8]] material, u are just seeing it early.
