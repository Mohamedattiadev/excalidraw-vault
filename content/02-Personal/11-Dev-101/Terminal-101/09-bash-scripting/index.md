...
[[02-Personal/11-Dev-101/Terminal-101/08-io-pipes-processes/index\|← Chapter 8]] | [[02-Personal/11-Dev-101/Terminal-101/index\|back to Terminal 101]]
...
> and u already made a script back in [[02-Personal/11-Dev-101/Terminal-101/07-scripts-env-path/index\|Chapter 7]], the `hello.sh` one with the shebang. so we start from there and we make it actually useful.
...
and u already know how to run it, from [[02-Personal/11-Dev-101/Terminal-101/05-permissions-sudo/index\|Chapter 5]] and [[02-Personal/11-Dev-101/Terminal-101/07-scripts-env-path/index\|Chapter 7]]:
...
> exactly the same as [[02-Personal/11-Dev-101/Terminal-101/01-command-line-basics/index\|Chapter 1]], nothing changes when u put them in a file:
...
> and remember the quotes from [[02-Personal/11-Dev-101/Terminal-101/01-command-line-basics/index\|Chapter 1]], because in a script u will hit this every day:
...
> look at that last one. `ls | wc -l` is the pipe from [[02-Personal/11-Dev-101/Terminal-101/08-io-pipes-processes/index\|Chapter 8]], and now its result is sitting in a variable. this is where the chapters start connecting to each other.
...
- careful: always put `"$1"` in double quotes when u use it. if someone passes u a file name with a space in it, the unquoted version breaks in exactly the way we saw in [[02-Personal/11-Dev-101/Terminal-101/01-command-line-basics/index\|Chapter 1]].
...
> numbers do not use `=` and `>`, they use letters. it looks strange at the beginning but there is a reason: `>` already means redirect, from [[02-Personal/11-Dev-101/Terminal-101/08-io-pipes-processes/index\|Chapter 8]], so bash could not use it here.
...
> this one u will use the most, and it connects straight back to [[02-Personal/11-Dev-101/Terminal-101/03-files-and-navigation/index\|Chapter 3]] and [[02-Personal/11-Dev-101/Terminal-101/05-permissions-sudo/index\|Chapter 5]]:
...
> remember `$?` from [[02-Personal/11-Dev-101/Terminal-101/08-io-pipes-processes/index\|Chapter 8]]? this is where it earns its place:
...
> and this is exactly what `&&` was doing for u in [[02-Personal/11-Dev-101/Terminal-101/08-io-pipes-processes/index\|Chapter 8]]. now u can see what was happening under it.
...
> and now put it together with the wildcard from [[02-Personal/11-Dev-101/Terminal-101/03-files-and-navigation/index\|Chapter 3]], and u have something u would actually use:
...
- `*.txt` is the wildcard from [[02-Personal/11-Dev-101/Terminal-101/03-files-and-navigation/index\|Chapter 3]]. remember the shell replaces it **before** the loop starts, so what the loop actually receives is `notes.txt old.txt`.
...
- this is just `mv` from [[02-Personal/11-Dev-101/Terminal-101/03-files-and-navigation/index\|Chapter 3]], renaming one file.
...
- and the quotes around `"$file"` are the ones from [[02-Personal/11-Dev-101/Terminal-101/01-command-line-basics/index\|Chapter 1]]. without them, a file called `my notes.txt` would arrive as 2 separate arguments and the `mv` would fail.
...
- `cd "$folder" || exit 1`, the `||` from [[02-Personal/11-Dev-101/Terminal-101/08-io-pipes-processes/index\|Chapter 8]]
- `mkdir -p` from [[02-Personal/11-Dev-101/Terminal-101/03-files-and-navigation/index\|Chapter 3]]
- `for file in *` with the wildcard from [[02-Personal/11-Dev-101/Terminal-101/03-files-and-navigation/index\|Chapter 3]]
...
- `$(ls documents | wc -l)`, the pipe from [[02-Personal/11-Dev-101/Terminal-101/08-io-pipes-processes/index\|Chapter 8]] inside a `$( )` from section 2
- `exit 1` sets the exit code from [[02-Personal/11-Dev-101/Terminal-101/08-io-pipes-processes/index\|Chapter 8]], so another script can check if this one worked
...
> stuck, or done and want to check? [[02-Personal/11-Dev-101/Terminal-101/09-bash-scripting/solutions\|the solutions are here]]
