...
[[11-Dev-101/Terminal-101/05-permissions-sudo/index\|← Chapter 5]] | [[11-Dev-101/Terminal-101/index\|back to Terminal 101]] | [[11-Dev-101/Terminal-101/07-scripts-env-path/index\|next: Chapter 7 →]]

---

## 1. why we need an editor at all

> until now, every time we wanted to see what was inside a file we used `cat`, `less`, `head` or `tail` from [[11-Dev-101/Terminal-101/03-files-and-navigation/index\|Chapter 3]]. all of those only **read** the file.

> and for writing into a file there is `>` and `>>`, which are coming in [[11-Dev-101/Terminal-101/08-io-pipes-processes/index\|Chapter 8]]. they do work, but try writing a 40 line script with `echo` and `>>` and u will see the problem after the third line.
...
> same as `grep` from [[11-Dev-101/Terminal-101/04-search-grep-find/index\|Chapter 4]] but inside the file u have open:
...
2. `nvim` (neovim) is a newer version of vim, and it comes up again in [[11-Dev-101/Terminal-101/08-io-pipes-processes/index\|Chapter 8]] with the package managers. everything in this chapter works exactly the same in it.
...
3. some commands open an editor for u instead of taking an argument, like `git commit`. which editor they open is decided by the `EDITOR` environment variable, and u set it with `export`, which is the next chapter, [[11-Dev-101/Terminal-101/07-scripts-env-path/index\|Chapter 7]]:
...
4. vim has its own config file, `~/.vimrc`, same idea as the `.bashrc` file we get to in [[11-Dev-101/Terminal-101/07-scripts-env-path/index\|Chapter 7]]. u don't need it yet, but that is where people's setups come from.
...
> stuck, or done and want to check? [[11-Dev-101/Terminal-101/06-text-editors/solutions\|the solutions are here]]
