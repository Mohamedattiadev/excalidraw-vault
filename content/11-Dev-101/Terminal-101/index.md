...
|---|---|---|---|
| 1 | [[11-Dev-101/Terminal-101/01-command-line-basics/index\|Command Line Basics]] | ~45 min | `echo`, CLI vs GUI, terminal / emulator / shell, variables, quotes, the `Tab` key |
| 2 | [[11-Dev-101/Terminal-101/02-the-linux-file-system/index\|The Linux File System]] | ~45 min | what the real tree is: `/etc` `/var` `/usr` `/tmp` `/home` and the rest |
| 3 | [[11-Dev-101/Terminal-101/03-files-and-navigation/index\|Files and Navigation]] | ~90 min | `cd` `ls` `pwd` `mkdir` `touch` `rm` `cp` `mv` `cat` `less` `head` `tail`, the `*` wildcard, `man` |
| 4 | [[11-Dev-101/Terminal-101/04-search-grep-find/index\|Searching: grep and find]] | ~60 min | `grep` and its flags, `find` and its patterns |
| 5 | [[11-Dev-101/Terminal-101/05-permissions-sudo/index\|Permissions and sudo]] | ~75 min | root, `sudo`, reading `-rw-r--r--`, `chmod`, `chown` |
| 6 | [[11-Dev-101/Terminal-101/06-text-editors/index\|Text Editors]] | ~60 min | `nano`, and `vim` with the modes idea behind it |
| 7 | [[11-Dev-101/Terminal-101/07-scripts-env-path/index\|Scripts, Env and PATH]] | ~75 min | compiled vs interpreted, shebang, `export`, `.bashrc`, aliases, `PATH` |
| 8 | [[11-Dev-101/Terminal-101/08-io-pipes-processes/index\|Input, Output and Processes]] | ~105 min | exit codes, `>` `>>` `2>`, the `\|` pipe, `&&`, background jobs, `ps` `kill` `top` |
| 9 | [[11-Dev-101/Terminal-101/09-bash-scripting/index\|Bash Scripting Basics]] | ~120 min | arguments, `if`, loops, `read`, functions, and a real 25 line script |

And when u forget something later: **[[11-Dev-101/Terminal-101/CHEATSHEET\|the cheat sheet]]**, every
...
- Forgetting a command later is normal and expected. That is what `man` and the
  [[11-Dev-101/Terminal-101/CHEATSHEET\|cheat sheet]] are for. U are learning where to look, not
  memorising a list.
...
[[11-Dev-101/index\|← back to dev-101]]
