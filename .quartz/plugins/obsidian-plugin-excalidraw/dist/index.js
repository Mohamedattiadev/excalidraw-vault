...
function isExcalidrawMd(content) {
  // Obsidian Excalidraw plugin marks .md files with this frontmatter line.
  return /^excalidraw-plugin:\s*\S/m.test(content);
}
function parseExcalidraw(content, filePath) {
  ...
  if (filePath.endsWith(".md") && isExcalidrawMd(content)) {
    return parseExcalidrawMd(content);
  }
  return null;
}
...
var excalidraw_inline_default = `function B(){let n=document.querySelector(".page[data-frame='excalidraw']");if(n){N(n);if(!n.querySelector(".exc-viewer-root"))q(n);return}let t=document.querySelectorAll(".excalidraw-page");for(let v of t){if(v.querySelector(".exc-viewer-root"))continue;q(v)}}function N(n){let t=n.querySelector(".excalidraw-sidebar-toggle");t&&(t.addEventListener("click",()=>{n.classList.toggle("excalidraw-sidebar-open")}),window.addCleanup(()=>{n.classList.remove("excalidraw-sidebar-open")}))}function q(n){let t=n.querySelector(".excalidraw-container");if(!t)return;let v=t.querySelector("svg");if(!v)return;t.style.backgroundColor="var(--excalidraw-bg, var(--light))";var h=n.querySelector(".excalidraw-overlays");let a=1,u=0,d=0,c=!1,m=0,M=0;function O(){if(h){var e=h.querySelectorAll(".excalidraw-overlay");if(e.length!==0){var o=parseFloat(h.getAttribute("data-offset-x"))||0,l=parseFloat(h.getAttribute("data-offset-y"))||0,r=v.getScreenCTM(),g=t.getBoundingClientRect();if(r)for(var p=0;p<e.length;p++){var i=e[p],A=parseFloat(i.getAttribute("data-x"))||0,Y=parseFloat(i.getAttribute("data-y"))||0,T=parseFloat(i.getAttribute("data-w"))||0,P=parseFloat(i.getAttribute("data-h"))||0,C=A+o,F=Y+l,k=C*r.a+r.e-g.left,z=F*r.d+r.f-g.top,D=T*r.a,I=P*r.d;i.style.left=k+"px",i.style.top=z+"px",i.style.width=D+"px",i.style.height=I+"px",i.style.display="flex"}}}}O();function s(){v.style.transform="translate("+u+"px, "+d+"px) scale("+a+")",O()}function y(e){e.preventDefault();var o=e.deltaY>0?-.2:.2;a=Math.max(.05,Math.min(50,a+o)),s()}function E(e){e.button===0&&(c=!0,m=e.clientX-u,M=e.clientY-d,t.style.cursor="grabbing")}function w(e){c&&(u=e.clientX-m,d=e.clientY-M,s())}function x(){c=!1,t.style.cursor="grab"}var L=n.querySelector(".excalidraw-zoom-in"),S=n.querySelector(".excalidraw-zoom-out"),b=n.querySelector(".excalidraw-reset");L&&L.addEventListener("click",function(){a=Math.min(50,a+.2),s()}),S&&S.addEventListener("click",function(){a=Math.max(.05,a-.2),s()}),b&&b.addEventListener("click",function(){a=1,u=0,d=0,s()});var f=0;function X(e){if(e.touches.length===1)c=!0,m=e.touches[0].clientX-u,M=e.touches[0].clientY-d;else if(e.touches.length===2){c=!1;var o=e.touches[0].clientX-e.touches[1].clientX,l=e.touches[0].clientY-e.touches[1].clientY;f=Math.sqrt(o*o+l*l)}}function Z(e){if(e.preventDefault(),e.touches.length===1&&c)u=e.touches[0].clientX-m,d=e.touches[0].clientY-M,s();else if(e.touches.length===2&&f>0){var o=e.touches[0].clientX-e.touches[1].clientX,l=e.touches[0].clientY-e.touches[1].clientY,r=Math.sqrt(o*o+l*l),g=r/f;a=Math.max(.05,Math.min(50,a*g)),f=r,s()}}function _(){c=!1,f=0}t.addEventListener("wheel",y,{passive:!1}),t.addEventListener("mousedown",E),document.addEventListener("mousemove",w),document.addEventListener("mouseup",x),t.addEventListener("touchstart",X,{passive:!0}),t.addEventListener("touchmove",Z,{passive:!1}),t.addEventListener("touchend",_),window.addCleanup(function(){t.removeEventListener("wheel",y),t.removeEventListener("mousedown",E),document.removeEventListener("mousemove",w),document.removeEventListener("mouseup",x),t.removeEventListener("touchstart",X),t.removeEventListener("touchmove",Z),t.removeEventListener("touchend",_)})}document.addEventListener("nav",B);
`;
var l3;
...
var ExcalidrawPage = (opts) => ({
  name: "ExcalidrawPage",
  priority: 25,
  fileExtensions: [".excalidraw.md", ".excalidraw", ".md"],
  match: excalidrawMatcher,
  generate({ ctx }) {
    const excalidrawFiles = ctx.allFiles.filter((fp) => {
      if (fp.endsWith(".excalidraw.md") || fp.endsWith(".excalidraw")) return true;
      if (!fp.endsWith(".md")) return false;
      try {
        const head = readFileSync(join(ctx.argv.directory, fp), "utf-8").slice(0, 2048);
        return /^excalidraw-plugin:\s*\S/m.test(head);
      } catch { return false; }
    });
  ...
  shouldPublish(_ctx, content) {
    ...
    // Keep the virtual page that carries excalidrawData (from generate()).
    if ("excalidrawData" in fileData) return true;
    const relativePath = fileData.relativePath ?? "";
    if (relativePath.endsWith(".excalidraw.md") || relativePath.endsWith(".excalidraw")) {
      return false;
    }
    const fm = fileData.frontmatter ?? {};
    if (fm["excalidraw-plugin"]) return false;
    const tags = fm.tags;
    if (Array.isArray(tags) && tags.includes("excalidraw")) return false;
    return true;
  },
  layout: "excalidraw",
  frame: "excalidraw",
...
export { ExcalidrawBody_default as ExcalidrawBody, ExcalidrawFrame, ExcalidrawPage, parseExcalidraw, parseExcalidrawJson, parseExcalidrawMd };
